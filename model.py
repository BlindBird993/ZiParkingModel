...
class ConceptModel(Model):
    ...
    number_of_car_agents = 0
    number_of_parking_slot_agents = 0
    ...
    def __init__(self, number_of_car_agents = 100,number_of_parking_slot_agents = 80):
        ...
        #initial price for parking slots
        price_agent = InitAgent(0,self)
        self.schedule.add(price_agent)

        #parking_slots
        for i in range(self.number_of_parking_slot_agents):
            slot_agent = ParkingSlotAgent("Parking slot {}".format(i),self)
            self.schedule.add(slot_agent)

        #car agents
        for i in range(self.number_of_car_agents):
            car_agent = CarAgent("Car agent {}".format(i), self)
